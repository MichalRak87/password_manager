...
class AddPassword:
    def __init__(self, db, _index, tab_sys,):
        self.crypto = None
        ...
        style.configure('TButton', background='#B5CDA3', foreground='white', width=10, borderwidth=5, focusthickness=5)
    ...
    def on_click(self, event, user_password):
        if all([len(self.portal_entry.get()), len(self.login_entry.get()), len(self.password_entry.get())]):
            self.crypto = Crypto(user_password)
            with Session(self.db) as session:
                portal = Portal(name=self.portal_entry.get())
                credentials = Credentials(login=self.login_entry.get(),
                ...
                session.commit()
                self.index.tree.insert("", "end", values=(credentials.portal.name, credentials.login))
            self.tab_sys.select(1)
            message = tkinter.messagebox.showinfo(title='Add credentials', message='New credential added successfully')
...
class IndexView:
    def __init__(self, db):
        ...
        style.configure("Treeview.Heading", font=("Comic Sans MS", 10),
                        background="#4C4C6D", foreground="yellow")

        style.configure("Treeview", font=("Comic Sans MS", 10),
                        background="#DEEDF0", foreground="black",
                        fieldbackground="#E8F6EF")


        self.fill_treeview()
        self.configure_treeview()
        self.tree.bind("<<TreeviewSelect>>", lambda x: self.on_click('', check.pin))
    ...
    def fill_treeview(self):
        with Session(self.db) as session:
            for credential in session.query(Credentials).all():
                ...
                self.tree.insert(
                    "", "end", values=(credential.portal, credential.login)
                )

    def on_click(self, event, user_password):
        self.crypto = Crypto(user_password)
        [selected_column] = self.tree.selection()
        selected_items = self.tree.item(selected_column, 'values')
        with Session(self.db) as session:
            credential = session.query(Credentials).join(Portal).filter(
                Portal.name == selected_items[0],
                Credentials.login == selected_items[1]
            ).one()
            try:
                decrypted = self.crypto.decrypt(credential.password)
                root.clipboard_clear()
                root.clipboard_append(decrypted)
                message = tkinter.messagebox.showinfo(title='Password', message='Password copied to clipboard')
...
if __name__ == "__main__":
    ...
    style.configure("TNotebook.Tab", background="white",
                    foreground="black", font=("Comic Sans MS", 10))
    ...
    style_frame.configure("TNotebook.Tab",
                          foreground="#293462", font=("Comic Sans MS", 10,))
